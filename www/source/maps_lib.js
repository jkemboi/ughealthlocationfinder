...
$.extend(MapsLib, {
    ...
    searchHtml: function() {
        ...
        if (settings.addressShow)
        {
            html.push("<head><link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css'> <meta name='viewport' content='width=device-width, initial-scale=1'> <style> .buttonload {  background-color: #4CAF50; border: none; color: white;  padding: 12px 24px; font-size: 16px; } /* Add a right margin to each icon */ .fa {margin-left: -12px; margin-right: 8px;} </style> </head><body><p>My Current location</p><button onclick='getLocation()'>Pick my position</button><p><script>function getLocation(){if (navigator.geolocation) <button class='buttonload'><i class='fa fa-spinner fa-spin'></i>Loading</button>{navigator.geolocation.getCurrentPosition(showPosition);} else {var x= 'Geolocation is not supported.';}}function showPosition(position) {var x = + position.coords.latitude + ", " + position.coords.longitude;document.getElementsByName('search_address')[0].value= x;}</script><label for='search_address'></label><input type='textbox' name='search_address'></input></body>");
